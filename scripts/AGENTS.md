...
4. **`scripts/release.mjs` publishes to npm.** Use `--dry-run` first. The publish order is: `@vesk/types` → `@vesk/compiler` → `@vesk/runtime` → `@vesk/plugin-tailwind` → `@vesk/adapter` → `@vesk/lsp` → `vesk` → `create-vesk`.
