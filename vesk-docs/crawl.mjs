/**
 * vesk-docs route crawler — full-capture audit.
 *
 * A docs-site port of the repo-level /root/vesk/crawl.mjs (which is hardwired
 * to test-app routes — do not touch it). This one discovers the docs routes
 * from the app directory + the markdown corpus and captures EVERYTHING:
 *
 ...
 *      node:internal frames, ssr-data script refs (must be 1 on data routes,
 *      0 everywhere else), x-vesk-data requests on initial load (must be 0)
 ...
 *    · x-vesk-data requests fire exactly once per visited route (200)
 *
 * Data routes (embed an ssr-data script exactly once) are /posts and every
 * /guide/* page — they fetch at render time via useFetch / useFetch.text.
 * All other pages are static SSR.
 *
 * The docs pages render an intentional error state when a /guide/:path has
 * no doc.md ("no source at this path") — that is a 200 page, not a 500; the
 * crawler expects 200 and checks for the error-panel content instead.
 *
...
// Discover the guide corpus: every public/docs/guide/**/doc.md becomes a
// /guide/:path route (matches the [...path] catch-all component).
function discoverGuidePaths() {
  ...
  const walk = (dir, base) => {
    for (const e of readdirSync(dir, { withFileTypes: true })) {
      ...
      else if (e.name === 'doc.md') paths.push(base.replace(/^\//, ''));
    }
  };
  walk(join(import.meta.dirname, 'public/docs/guide'), '');
  return paths.sort();
}

const GUIDE = discoverGuidePaths();

// Static SSR routes (no fetch, no ssr-data script).
const ROUTES = [
  { route: '/' },
  { route: '/about' },
  { route: '/blog' },
  ...
  { route: '/blog/no-such-slug' },          // dynamic [slug] — renders for any slug
  { route: '/guide' },                        // guide index (<Md> server-fetch, static SSR)
  // useFetch.text data routes — SSR data payload + client fetch of doc.md:
  ...GUIDE.map(p => ({ route: `/guide/${p}`, isData: true })),
  // A missing guide doc is a 200 with the "no source at this path" panel.
  { route: '/guide/no-such-doc', docMissing: true },
  { route: '/posts', isData: true },          // useFetch({ into }) demo
  { route: '/statements' },
  { route: '/does-not-exist', expectStatus: 404 }, // custom not-found.vsk
];

// Phase B SPA-navigation content marker per route. Guide docs assert the
// "route: /guide/…" doc-meta line plus the rendered markdown.
const SPA_TEXT = {
  '/': 'what the compiler emits',
  '/about': 'How it works',
  '/blog': 'Hello, world',
  '/blog/hello-world': 'route /blog/hello-world',
  '/blog/ssr-in-vesk': 'route /blog/ssr-in-vesk',
  '/blog/no-such-slug': 'route /blog/no-such-slug',
  '/guide': 'route: /guide',
  '/posts': 'Posts, fetched live',
  '/statements': 'Every JS construct',
  '/does-not-exist': 'not found',
};
for (const p of GUIDE) SPA_TEXT[`/guide/${p}`] = `route: /guide/${p}`;
SPA_TEXT['/guide/no-such-doc'] = 'no source at this path';

// Per-route x-vesk-data expectations during SPA navigation: the router
// fetches SSR props via X-Vesk-Data for EVERY SPA navigation (count 1, 200).
...
function analyzeDocument(body, entry, docStatus) {
  ...
  const wantRefs = entry.docMissing || entry.expectStatus === 404 ? 1 : (entry.isData ? 1 : 0);
  if (refs !== wantRefs) {
  ...
  if (entry.docMissing && !body.includes('no source at this path')) {
    leaks.push('missing doc should render the "no source at this path" panel');
  }
  return { leaks, fail: leaks.some(l => !l.includes('(tolerated)')) };
}

// Validate an ssr-data.js payload: must be a real `globalThis.__vsk_ssr_data =
// {...}` JSON assignment whose values are data, not error/HTML leakage.
...
async function fullLoad(entry) {
  ...
  try {
    ...
    // Only pages that load the dev client (/about hydration via /_vesk/client.js;
    // prod serves it at /_vesk/static/client.js) have anything to hydrate.
    if (!loadError && (body.includes('/_vesk/client.js') || body.includes('_vesk/static/client.js'))) {
  ...
  if (entry.isData && !loadError && docStatus === 200) {
  ...
  const row = { route, statusStr, hydrationMs, pgErr: cap.pageerrors.length, cErr: realConsole.length, warn: cap.console.filter(m => m.type === 'warning').length, reqFail: cap.requestfailed.length, xvesk: cap.xvesk.length, leak: docLeak.leaks.join('; ') || '-', problems, cap };
  await page.close();
  return row;
}
...
async function spaChain() {
  ...
  // SPA-navigating all 40+ guide pages in one session is slow but thorough.
  ...
  if (hasRouter) {
    ...
    for (const route of chain) {
      ...
      else if (statuses.some(s => s !== SPA_XVESK_DEFAULT.status)) {
        row.problems.push(`x-vesk-data status(es) ${statuses.join(',')} (expected ${SPA_XVESK_DEFAULT.status})`);
      }
...
async function main() {
  ...
  console.log('route'.padEnd(40), 'TTFB(ms)', 'total(ms)', 'http', 'bytes');
  for (const r of ssrRows) {
    console.log(r.route.padEnd(40), String(r.ttfb).padEnd(9), String(r.total).padEnd(9), r.code.padEnd(5), r.bytes);
  }

  // ── Phase A: full loads, capture everything ──
  console.log('\n=== Phase A: fresh full load per route ===');
  console.log('route'.padEnd(40), 'status'.padEnd(9), 'hyd(ms)'.padEnd(8), 'pgErr'.padEnd(6), 'cErr'.padEnd(5), 'warn'.padEnd(5), 'reqFail'.padEnd(8), 'xvesk'.padEnd(6), 'leaks');
  const rows = [];
  for (const entry of ROUTES) {
    ...
    console.log(row.route.padEnd(40), row.statusStr.padEnd(9), String(row.hydrationMs).padEnd(8), String(row.pgErr).padEnd(6), String(row.cErr).padEnd(5), String(row.warn).padEnd(5), String(row.reqFail).padEnd(8), String(row.xvesk).padEnd(6), (ok ? '  \u2713' : '  \u2717 ' + row.problems.join('; ')));
  }
  ...
  if (!spa.hasRouter) {
    ...
    console.log('route'.padEnd(40), 'xvesk'.padEnd(6), 'verdict');
    for (const row of spa.rows) {
      verdict(row.ok, `Phase B ${row.route}`, `[B] ${row.route}: ${row.problems.join('; ')}`);
      console.log(row.route.padEnd(40), String(row.xvesk ?? '-').padEnd(6), row.ok ? '  \u2713' : '  \u2717 ' + row.problems.join('; '));
    }
  }
