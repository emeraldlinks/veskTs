...
function findRuntimeSrc(appDir) {
  ...
  for (const base of candidates) {
    for (const dir of [base, join2(base, "dist")]) {
      if (existsSync2(join2(dir, "index-client.js")))
        return dir;
    }
  }
...
function buildRuntimeCode(runtimeDir2) {
  ...
  for (const f of runtimeFiles) {
    const p = join2(runtimeDir2, f);
    if (existsSync2(p)) {
      let src = readFileSync2(p, "utf-8");
      src = stripTypes(src);
...
`;
  ...
  const indexSrc = readFileSync2(join2(runtimeDir2, "index-client.js"), "utf-8");
...
async function buildTreeShakenRuntime(runtimeDir2, usedNames) {
  ...
  const entry = join2(runtimeDir2, `.runtime-tree-entry-${runtimeEntryId++}.mjs`);
  try {
...
`;
    ...
    const runtimeGlobals2 = globalNames.filter((n) => importedSet.has(n)).map((n) => `globalThis.${n} = ${n};
`).join("") + "globalThis.__runtime_comps = __runtime_comps;\nglobalThis.__cleanup = __cleanup;\nglobalThis.__place = __place;\n\n";
...
function resolveSpec(spec, fromFile, dirs) {
...
function literalName(n) {
  return n.name !== void 0 ? n.name : String(n.value);
}
...
function directDeclNames(stmts) {
  const out = [];
...
function importRefText(name, importMap) {
...
function pushScope(names, scopes) {
  scopes.push(new Set(names));
}
function inScope(name, scopes) {
  for (let i2 = scopes.length - 1; i2 >= 0; i2--) if (scopes[i2].has(name)) return true;
  return false;
}
function collectImportEdits(stmts, importMap, edits, source) {
  const scopes = [];
  ...
  const printRewrite = (node) => {
    ...
    switch (node.type) {
      ...
      case "OptionalCallExpression":
        return `${printRewrite(node.callee)}(${(node.arguments || []).map((a) => printRewrite(a)).join(", ")})`;
      case "NewExpression":
        return `new (${printRewrite(node.callee)})(${(node.arguments || []).map((a) => printRewrite(a)).join(", ")})`;
      case "ChainExpression":
    ...
    switch (node.type) {
      ...
      case "ForStatement":
        if (node.init && node.init.type === "VariableDeclaration") {
          pushScope([].concat(...node.init.declarations.map((d) => bindingNames(d.id))), scopes);
      ...
      case "ForOfStatement":
        if (node.left.type === "VariableDeclaration") {
          pushScope([].concat(...node.left.declarations.map((d) => bindingNames(d.id))), scopes);
      ...
      case "TryStatement":
        ...
        scopes.pop();
        if (node.handler) {
          pushScope([].concat(node.handler.param ? bindingNames(node.handler.param) : []), scopes);
...
function splice(source, removedRanges, edits) {
...
function transformModule(filePath, source, dirs) {
  let ast;
  try {
    ast = parse3(source, { sourceType: "module", ecmaVersion: "latest" });
  } catch (e) {
    throw new MiniBundleError(`vesk: mini-bundle failed to parse ${filePath}: ${e.message}`);
  }
  ...
  for (const stmt of ast.body) {
    switch (stmt.type) {
      case "ImportDeclaration": {
        ...
        if (id === null) {
          const ext = { spec, named: [], namespaces: [], defaults: [], sideEffect: false };
          ...
          if (id === null) {
            for (const s of stmt.specifiers || []) {
              ...
              externalReexports.push(
                local === exported ? `export { ${local} } from ${JSON.stringify(spec)};` : `export { ${local} as ${exported} } from ${JSON.stringify(spec)};`
              );
            }
          } else {
            addDep(id);
...
function collectGraph(entryIds, dirs) {
...
function emitExternals(externals) {
  const out = [];
...
function emitRegistry(modules, indent) {
  const pad = " ".repeat(indent);
  const lines = [`${pad}const __registry = {`];
...
function emitPreloads(order, indent) {
  const pad = " ".repeat(indent);
  return order.map((id) => `${pad}__load(${JSON.stringify(id)});`).join("\n");
}
function hubExportMap(runtimeDir2) {
  const hubPath = join(runtimeDir2, "index-client.js");
  if (!existsSync(hubPath)) {
    throw new MiniBundleError(`vesk: runtime hub not found at ${hubPath}`);
  }
  const ast = parse3(readFileSync(hubPath, "utf-8"), { sourceType: "module", ecmaVersion: 2022 });
...
function bundleClientRuntimeIife(runtimeDir2, usedNames) {
  const hubPath = join(runtimeDir2, "index-client.js");
  const hub = hubExportMap(runtimeDir2);
  const unique = [...new Set(usedNames)];
  const missing = unique.filter((n) => !hub.has(n));
  if (missing.length > 0) {
    throw new MiniBundleError(`runtime names not exported by @vesk/runtime: ${missing.join(", ")}`);
  }
  const dirs = { runtimeDir: runtimeDir2 };
  const entryIds = [];
  const usedGetters = [];
...
function bundleServerRuntime(runtimeDir2, compilerDir2, entryPath) {
  const dirs = { runtimeDir: runtimeDir2, compilerDir: compilerDir2 };
...
function postprocessClientCode(source) {
  ...
  while (i2 < tokens.length) {
    const t = tokens[i2];
    if (depth === 0 && t.type === types$1._import && i2 + 1 < tokens.length && tokens[i2 + 1].type !== types$1.parenL) {
      const end = scanImportEnd(tokens, i2);
      const sourceStr = importSource(tokens, i2, end);
      if (sourceStr === "@vesk/runtime") {
        for (const name of importSpecifierNames(tokens, i2, end)) {
          if (!seen.has(name)) {
            seen.add(name);
    ...
    if (depth === 0 && t.type === types$1._function && i2 + 1 < tokens.length) {
      const nameTok = tokens[i2 + 1];
      if (nameTok.type === types$1.name && (nameTok.value === "__cleanup" || nameTok.value === "__place")) {
        const end = scanFunctionEnd(tokens, i2);
        removed.push({ start: t.start, end: tokens[end - 1].end });
        i2 = end;
        continue;
      }
    ...
    if (depth === 0 && isDeclKeyword(t)) {
      const end = scanStmtEnd(tokens, i2);
      if (isEmptyComponentsDecl(tokens, i2, end)) {
        removed.push({ start: t.start, end: tokens[end - 1].end });
      }
      i2 = end;
      continue;
    }
    if (depth === 0 && t.type === types$1._export) {
      const end = scanStmtEnd(tokens, i2);
      if (isComponentsExport(tokens, i2, end)) {
        removed.push({ start: t.start, end: tokens[end - 1].end });
      }
      i2 = end;
      continue;
...
function rewriteRuntimeImportSources(source) {
  const tokens = tokenize(source);
  if (tokens.length === 0) {
  ...
  while (i2 < tokens.length) {
    const t = tokens[i2];
    if (t.type === types$1._import && i2 + 1 < tokens.length && tokens[i2 + 1].type !== types$1.parenL) {
      const end = scanImportEnd(tokens, i2);
      const srcTok = importSourceToken(tokens, i2, end);
    ...
    if (t.type === types$1._export) {
      const end = scanStmtEnd(tokens, i2);
      const srcTok = exportFromSourceToken(tokens, i2, end);
      if (srcTok && isRuntimeSpecifier(srcTok.value)) {
        edits.push([srcTok.start, srcTok.end, "'../runtime.js'"]);
      }
      i2 = end;
...
function isRuntimeSpecifier(value) {
  return value === "@vesk/runtime" || value.startsWith("@vesk/runtime/");
}
function tokenize(source) {
  let tokens = [];
  try {
    const tok = tokenizer2(source, { ecmaVersion: "latest", sourceType: "module" });
    while (true) {
      tok.next();
      if (tok.type === types$1.eof) break;
...
function isOpen(t) {
  return t.type === types$1.braceL || t.type === types$1.bracketL || t.type === types$1.parenL || t.type === types$1.dollarBraceL;
}
function isClose(t) {
  return t.type === types$1.braceR || t.type === types$1.bracketR || t.type === types$1.parenR;
}
function isDeclKeyword(t) {
  return t.type === types$1._const || t.type === types$1._var || t.type === types$1.name && t.value === "let";
}
function scanStmtEnd(tokens, start) {
  let depth = 0;
  for (let i2 = start + 1; i2 < tokens.length; i2++) {
    const t = tokens[i2];
    if (isOpen(t)) depth++;
    else if (isClose(t)) depth--;
    if (t.type === types$1.semi && depth <= 0) return i2 + 1;
  }
  return tokens.length;
}
function scanImportEnd(tokens, start) {
  let depth = 0;
  for (let i2 = start + 1; i2 < tokens.length; i2++) {
    const t = tokens[i2];
    if (t.type === types$1.string) {
      return i2 + 1;
    }
    if (isOpen(t)) depth++;
    else if (isClose(t)) depth--;
...
function scanFunctionEnd(tokens, start) {
  let depth = 0;
  for (let i2 = start + 1; i2 < tokens.length; i2++) {
    const t = tokens[i2];
    if (isOpen(t)) depth++;
      ...
      depth--;
      if (t.type === types$1.braceR && depth === 0) return i2 + 1;
    }
  }
  return tokens.length;
}
function isFromToken(t) {
  return t.type === types$1.name && t.value === "from";
}
function importSourceToken(tokens, start, end) {
  for (let i2 = start + 1; i2 < end; i2++) {
    const t = tokens[i2];
    if (t.type === types$1.string) {
      if (isFromToken(tokens[i2 - 1])) return t;
      if (i2 === start + 1) return t;
      return null;
    }
  }
  return null;
}
function importSource(tokens, start, end) {
  const t = importSourceToken(tokens, start, end);
  return t ? t.value : null;
}
function exportFromSourceToken(tokens, start, end) {
  for (let i2 = start + 1; i2 + 1 < end; i2++) {
    if (isFromToken(tokens[i2]) && tokens[i2 + 1].type === types$1.string) {
      return tokens[i2 + 1];
    }
  }
  return null;
}
function importSpecifierNames(tokens, start, end) {
  const names = [];
  let inSpecs = false;
  let specStart = -1;
  for (let i2 = start + 1; i2 < end; i2++) {
    const t = tokens[i2];
    if (t.type === types$1.braceL) {
      inSpecs = true;
      specStart = i2 + 1;
      continue;
    }
    if (t.type === types$1.braceR) {
      collectSpecifierGroup(tokens, specStart, i2, names);
      inSpecs = false;
      continue;
    }
    if (inSpecs && t.type === types$1.comma) {
      collectSpecifierGroup(tokens, specStart, i2, names);
      specStart = i2 + 1;
    }
  }
...
function collectSpecifierGroup(tokens, start, end, out) {
  let firstWord = null;
  for (let i2 = start; i2 < end; i2++) {
    const t = tokens[i2];
    if (t.type === types$1._typeof || t.type === types$1.name && t.value === "typeof") return;
    if (t.type === types$1.name) {
      firstWord = t.value;
      break;
    }
  }
...
function isEmptyComponentsDecl(tokens, start, end) {
  if (end - start !== 6) return false;
  if (tokens[start + 1].type !== types$1.name) return false;
  const id = tokens[start + 1].value;
  if (id !== "__components" && id !== "__hydrators") return false;
  if (tokens[start + 2].type !== types$1.eq) return false;
  if (tokens[start + 3].type !== types$1.braceL) return false;
  if (tokens[start + 4].type !== types$1.braceR) return false;
  return tokens[start + 5].type === types$1.semi;
}
function isComponentsExport(tokens, start, end) {
  if (tokens[start + 1].type === types$1._default) {
    return isComponentsRefAt(tokens, start + 2);
  }
  if (isDeclKeyword(tokens[start + 1])) {
    if (tokens[start + 2].type !== types$1.name) return false;
    if (tokens[start + 3].type !== types$1.eq) return false;
    return isComponentsRefAt(tokens, start + 4);
  }
  return false;
}
function isComponentsRefAt(tokens, idx) {
  if (idx + 1 >= tokens.length) return false;
  const t = tokens[idx];
  if (t.type !== types$1.name || t.value !== "__components") return false;
  return tokens[idx + 1].type === types$1.bracketL;
}
...
function isComponentsRefExpr(expr) {
  return expr?.type === "MemberExpression" && expr.computed === true && expr.object?.type === "Identifier" && expr.object.name === "__components";
}
function isEmptyComponentsDeclAst(decl) {
...
function postprocessViaAst(source) {
  let ast;
  try {
    ast = parse4(source, {});
  } catch {
...
function isComponentsNamedExport(decl) {
  if (!decl || decl.type !== "VariableDeclaration" || decl.declarations?.length !== 1) return false;
  return isComponentsRefExpr(decl.declarations[0]?.init);
}
function rewriteRuntimeImportsViaAst(source) {
  let ast;
  try {
    ast = parse4(source, {});
  } catch {
...
function resolveInstalledPackage(spec) {
  try {
    return dirname3(require2.resolve(`${spec}/package.json`));
  } catch {
    return "";
  }
}
function findCompilerSrc() {
  ...
  for (const base of candidates) {
    for (const dir of [base, join3(base, "dist")]) {
      if (existsSync3(join3(dir, "index.js")) || existsSync3(join3(dir, "index.ts"))) return dir;
    }
  }
...
function findRuntimeSrc2() {
  ...
  for (const base of candidates) {
    for (const dir of [base, join3(base, "dist")]) {
      if (existsSync3(join3(dir, "index-client.js")) || existsSync3(join3(dir, "index-client.ts"))) return dir;
    }
  }
...
function loadCompilerModule(name) {
  ...
  const path = existsSync3(tsPath) ? tsPath : jsPath;
  return import(path);
}
var compileClient2;
var compileServer;
...
async function processCssWithPlugins(css, filePath, projectDir) {
  ...
  let plugins = [];
  if (existsSync3(veskConfigPath) || existsSync3(veskConfigTsxPath)) {
    const configPath = existsSync3(veskConfigTsxPath) ? veskConfigTsxPath : veskConfigPath;
    try {
...
function serveStatic(req, res, filePath, contentType) {
  ...
  const data2 = readFileSync3(filePath, "utf-8");
...
function resolveRuntimeDir(projectDir) {
  ...
  if (existsSync3(join3(pkgDir, "ripple-runtime.js"))) return pkgDir;
  const distDir = join3(pkgDir, "dist");
  if (existsSync3(join3(distDir, "ripple-runtime.js"))) return distDir;
  if (existsSync3(join3(runtimeDir, "ripple-runtime.js"))) return runtimeDir;
  return null;
}
function loadEnvFiles(projectDir) {
  const files = [join3(projectDir, ".env"), join3(projectDir, ".env.local")];
...
async function loadConfig(projectDir) {
  ...
  const jsPath = join3(projectDir, "vesk.config.js");
  const tsPath = join3(projectDir, "vesk.config.ts");
  let configPath = null;
  if (existsSync3(jsPath)) configPath = jsPath;
  else if (existsSync3(tsPath)) configPath = tsPath;
  if (!configPath) return {};
  let raw;
  if (configPath.endsWith(".ts")) {
    const { transpile } = await import("typescript");
    const src = readFileSync3(configPath, "utf-8");
...
` + js;
    const tmpFile = join3(projectDir, ".vesk", "config.tmp.js");
    mkdirSync(dirname3(tmpFile), { recursive: true });
...
function stripTailwindDirectives(css) {
  ...
  while (i2 < lines.length) {
    ...
    if (TAILWIND_BLOCK.test(line)) {
      let braceCount = (lines[i2].match(/\{/g) || []).length - (lines[i2].match(/\}/g) || []).length;
      i2++;
      while (i2 < lines.length && braceCount > 0) {
        braceCount += (lines[i2].match(/\{/g) || []).length;
        braceCount -= (lines[i2].match(/\}/g) || []).length;
        i2++;
      }
      continue;
...
function prodStoreDataScript(payload) {
  if (!payload.props && !payload.ssrData) return null;
  const token = Math.random().toString(36).slice(2) + Date.now().toString(36);
  ...
  if (keys.length > 100) {
    for (let i2 = 0; i2 < keys.length - 100; i2++) delete store[keys[i2]];
  }
...
function registerSource(sourcePath) {
  if (!existsSync3(sourcePath)) return;
  try {
    devMods.compileFile(readFileSync3(sourcePath, "utf-8"), { sourcePath });
  } catch {
  }
}
function ensureActionRegistered(actionId, pagePathname, appDirPath, routeTree) {
  ...
  if (match) {
    for (let i2 = match.nodes.length - 1; i2 >= 0; i2--) {
      registerSource(resolve2(appDirPath, match.nodes[i2].sourceDir, "page.vsk"));
      registerSource(resolve2(appDirPath, match.nodes[i2].sourceDir, "layout.vsk"));
    }
  }
  if (devMods.getAction(actionId)) return;
...
async function renderPageHtml(pagePathname, params) {
  ...
  for (let i2 = chain.length - 1; i2 >= 0; i2--) {
    ...
    if (i2 === chain.length - 1 && node.page && existsSync3(pageFilePath)) {
      const src2 = readFileSync3(pageFilePath, "utf-8");
    ...
    if (node.layout && existsSync3(layoutFilePath)) {
      const src2 = readFileSync3(layoutFilePath, "utf-8");
  ...
  const hasLayout = chain.some((n) => n.layout && existsSync3(resolve2(appDirPath, n.sourceDir, "layout.vsk")));
  if (hasLayout) {
...
${devMods.prettifyHtml(body)}
  ...
  if (!leaf) return null;
  const src = readFileSync3(resolve2(appDirPath, leaf.sourceDir, "page.vsk"), "utf-8");
...
async function handleDevApi(req, url, bodyBuffer) {
  ...
  const apiDirPath = join3(appDirPath, "api");
  if (!existsSync3(apiDirPath)) return null;
  ...
  async function renderSSR() {
    return devMods.withSsrStore(async () => {
      ...
      for (let i2 = chain.length - 1; i2 >= 0; i2--) {
        ...
        const layoutFilePath = resolve2(appDirPath, node.sourceDir, "layout.vsk");
        if (i2 === chain.length - 1 && node.page && existsSync3(pageFilePath)) {
          const src = readFileSync3(pageFilePath, "utf-8");
        ...
        if (node.layout && existsSync3(layoutFilePath)) {
          const src = readFileSync3(layoutFilePath, "utf-8");
...
${devMods.prettifyHtml(body)}
  ...
  if (!match) {
    ...
    if (rootNode && rootNode.notFound) {
      ...
      if (existsSync3(nfPath)) {
        try {
          const nfSrc = readFileSync3(nfPath, "utf-8");
...
function readRawCss(projectDir) {
  const cssPath = join3(projectDir, "src", "global.css");
  const altCssPath = join3(projectDir, "src", "app.css");
  if (existsSync3(cssPath)) return { raw: readFileSync3(cssPath, "utf-8"), cssPath };
  if (existsSync3(altCssPath)) return { raw: readFileSync3(altCssPath, "utf-8"), cssPath: altCssPath };
  return { raw: "", cssPath };
}
...
function hmrClientJs() {
  const candidates = [
    join3(devState.runtimeDir, "hmr-client.js"),
    join3(devState.runtimeDir, "hmr-client.ts"),
    join3(runtimeDir, "hmr-client.js")
  ];
...
function richErrorPayload(err2, fullPath, errorMessage) {
  ...
  let code = "";
  if (line > 0 && existsSync3(fullPath)) {
    try {
      const src = readFileSync3(fullPath, "utf-8");
      const lines = src.split("\n");
      const start = Math.max(0, line - 3);
      const end = Math.min(lines.length, line + 2);
      code = lines.slice(start, end).map((l, i2) => `${start + i2 + 1}: ${l}`).join("\n");
    } catch {
    }
  }
  const tips = [];
...
async function devInit(params) {
  ...
  const apiCount = countFilesNamed(join3(appDir, "api"), "route.ts");
  return {
    ok: true,
...
function prodMatchPath(pattern, pathname) {
  ...
  let pi = 0, pp2 = 0;
  const params = {};
  while (pi < pathParts.length && pp2 < patternParts.length) {
    if (patternParts[pp2].startsWith(":")) {
      params[patternParts[pp2].slice(1)] = pathParts[pi];
      pi++;
      pp2++;
    } else if (patternParts[pp2] === pathParts[pi]) {
      pi++;
      pp2++;
    } else {
      return null;
  ...
  if (pp2 === patternParts.length && pi === pathParts.length) return params;
  return null;
}
...
async function prodInit(params) {
  ...
  const configPath = join3(outDir, "config.json");
  if (!existsSync3(configPath)) throw new Error(`no build found at ${outDir}`);
  const buildConfig = JSON.parse(readFileSync3(configPath, "utf-8"));
  let securityConfig = {};
  try {
    const config = await loadConfig(projectDir);
  ...
  const mwPath = join3(outDir, "server", "middleware.js");
  if (existsSync3(mwPath)) {
    try {
...
async function prodLoadFunction(funcPath) {
  ...
  if (!existsSync3(fullPath)) return null;
  try {
...
async function prodRenderNotFound(urlPath) {
  const state = prodState;
  const appDir = join3(state.projectDir, "app");
  const nfPath = join3(appDir, "not-found.vsk");
  if (!existsSync3(nfPath)) return null;
  try {
    const rtMod = await getProdRtMod();
    const src = readFileSync3(nfPath, "utf-8");
...
var notFoundHtmlCache = null;
var notFoundHtmlComputed = false;
async function prodRenderNotFoundCached(urlPath) {
...
async function prodRenderError(props) {
  const state = prodState;
  const appDir = join3(state.projectDir, "app");
  const errPath = join3(appDir, "error.vsk");
  if (!existsSync3(errPath)) return null;
  try {
    const rtMod = await getProdRtMod();
    const src = readFileSync3(errPath, "utf-8");
...
var prodIpStore = new AsyncLocalStorage();
function withProdClientIp(clientIp, fn) {
  return prodIpStore.run({ clientIp: clientIp || "127.0.0.1" }, fn);
}
function installSsrFetchHook() {
  globalThis.__vesk_ssr_fetch = (input, init) => {
    const state = prodState;
    const fallback = () => fetch(input, init);
    if (!state) return fallback();
    const baseUrl = globalThis.__vesk_ssr_base_url;
    const urlStr = typeof input === "string" ? input : input instanceof URL ? input.href : input && input.url || "";
    let target;
    try {
      target = new URL(urlStr, baseUrl || `http://localhost:${state.port || 3e3}`);
    } catch {
      return fallback();
    }
    const ownOrigin = (() => {
      try {
        return baseUrl ? target.origin === new URL(baseUrl).origin : target.origin === `http://localhost:${state.port || 3e3}`;
      } catch {
        return false;
      }
    })();
    ...
    const headers = {};
    try {
      new Headers(init && init.headers || req && req.headers || {}).forEach((v, k) => {
        headers[k] = v;
      });
    } catch {
    }
    return (async () => {
      let bodyB64;
      try {
        const rawBody = init && init.body !== void 0 && init.body !== null ? init.body : req && req.body !== null ? req.body : void 0;
...
async function handleProdRequest(p) {
  ...
  const staticDir = join3(state.outDir, "static");
  const publicDir = join3(staticDir, "public");
  ...
  if (url.pathname === "/_vesk/runtime.js") {
    const clientPath = join3(staticDir, "client.js");
  ...
  if (state.buildConfig.prerendered) {
    ...
    if (prerendered) {
      const htmlPath = join3(state.outDir, prerendered.file);
...
var server = createServer((req, res) => {
  ...
  if (req.method === "GET" && req.url === "/runtime.js") {
    ...
    for (const p of candidates) {
      if (existsSync3(p)) return serveStatic(req, res, p, "application/javascript");
    }
    res.writeHead(404);
      ...
      try {
        ...
        switch (rpcReq.method) {
          case "compile_client": {
            ...
            const code = compileClient2(source, filePath || null, opts);
            if (opts.postprocess) {
          ...
          case "compile_middleware_code": {
            ...
            for (let i2 = 0; i2 < (sources || []).length; i2++) {
              const extracted = extractMiddlewareParts(sources[i2]);
              if (!extracted) continue;
              parts.push(`async function mw_${i2}(${extracted.params}) {
${extracted.body}
}`);
            }
            if (parts.length === 0) {
            ...
            const code = [
              ...
              parts.join("\n\n"),
              "",
              `const __mwChain = [${parts.map((_, i2) => `mw_${i2}`).join(", ")}];`,
              "",
          ...
          case "bundle_runtime_iife": {
            const { runtimeDir: rd, usedNames } = rpcReq.params[0] || {};
            const code = bundleClientRuntimeIife(rd || runtimeDir, usedNames || []);
            respond({ jsonrpc: "2.0", id: rpcReq.id, result: { code } });
            return;
          }
          case "bundle_server_runtime": {
            const { runtimeDir: rd, compilerDir: cd, entryPath } = rpcReq.params[0] || {};
          ...
          case "collect_action_ids": {
            ...
            for (const p of paths || []) {
              ...
              try {
                const src = readFileSync3(p, "utf-8");
