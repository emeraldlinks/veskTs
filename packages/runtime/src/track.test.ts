...
function expect(value: unknown) {
  return {
    ...
    toHaveLength(n: number) {
      if ((value as { length: number }).length !== n) throw new Error(`Expected length ${n}, got ${(value as { length: number }).length}`);
    },
  };
}
...
describe('Derived', () => {
  it('creates a computed value', () => {
    root(() => {
      const count = track(5);
      const doubled = derived(() => (get(count) as number) * 2);
  ...
  it('chains derived values', () => {
    root(() => {
      const a = track(1);
      const b = derived(() => (get(a) as number) + 1);
      const c = derived(() => (get(b) as number) + 1);
      expect(get(c)).toBe(3);
      set(a, 10);
