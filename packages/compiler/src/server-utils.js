...
const RAW_TEXT_ELEMENTS = /* @__PURE__ */ new Set(["style", "script", "title"]);
let __vskHydrate = false;
let __vskId = 0;
let __vskImportedNames = null;
function resetVskState(hydrate = false) {
  __vskHydrate = hydrate;
  __vskId = 0;
}
function setVskImportedNames(names) {
...
function isStatic(body) {
  for (const node of body) {
    if (node instanceof StaticNode) {
      if (!isStatic(node.children)) return false;
...
function setRedactLogging(enabled) {
  _redactEnabled = enabled;
}
if (typeof console !== "undefined") {
  console.error = function(...args) {
    _origConsoleError.apply(console, args.map(
      (a) => typeof a === "string" && _redactEnabled ? redactLog(a) : a
    ));
  };
  console.log = function(...args) {
    _origConsoleLog.apply(console, args.map(
      (a) => typeof a === "string" && _redactEnabled ? redactLog(a) : a
    ));
  };
}
...
const __csrfSecrets = /* @__PURE__ */ new Map();
...
function csrfGuard(request, host) {
  if (!request || typeof request !== "object") return;
  const method = (request.method || "GET").toUpperCase();
  if (method === "GET" || method === "HEAD" || method === "OPTIONS") return;
  const headers = request.headers;
  const token = headers?.["x-csrf-token"] || request.body?._csrf || "";
  const requestHost = host || headers?.["host"] || "";
  if (!verifyCsrfToken(token, requestHost)) {
    throw new Error("CSRF validation failed");
  }
}
const __cookieSecrets = /* @__PURE__ */ new Map();
...
async function signCookie(name, value, host) {
...
async function unsignCookie(name, signedValue, host) {
...
async function setSignedCookie(name, value, options = {}, host) {
...
async function readSignedCookie(name, cookieString, host) {
...
function securityHeaders(config = {}) {
  ...
  if (sec.contentSecurityPolicy !== false) {
    headers["Content-Security-Policy"] = sec.contentSecurityPolicy || "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; frame-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'";
  }
  return headers;
}
function corsHeaders(security = {}, requestOrigin = "", host = "") {
...
function corsPreflight(request, security) {
...
function securityComment(config = {}) {
  const sec = config.security || {};
  const features = [];
    ...
    middleware(request, response) {
      ...
      if (!_check(ip)) {
        if (response && typeof response.headers === "object") {
          response.headers["Retry-After"] = String(Math.ceil(windowMs / 1e3));
        }
        return false;
      }
      return true;
...
function getClientIp(request, trustProxy = false) {
  ...
  if (headers["x-real-ip"]) return typeof headers["x-real-ip"] === "string" ? headers["x-real-ip"] : String(headers["x-real-ip"]);
  return "unknown";
}
function getClientProtocol(request, trustProxy = false) {
  if (trustProxy) {
...
function applyTrustProxy(ctx, trustProxy) {
...
function tryEvalExpr(raw2, props, locals = {}) {
  try {
    const fn = new Function("props", "return (" + raw2 + ")");
    return fn(props);
  } catch {
    ...
    try {
      ...
      try {
        const fn = new Function(...paramNames, "return (" + raw2 + ")");
        return fn(...paramValues);
      } catch {
        throw new Error("Cannot evaluate: " + raw2);
      }
    }
  }
}
...
let __cachedRuntimeModule = {};
try {
    __cachedRuntimeModule = await import('@vesk/runtime/server');
} catch {
    // runtime module not available
}
function setRuntimeModule(mod) {
...
async function callStaticProps(fnSource) {
  ...
  const wrapper = isAsync ? `return (async () => {
${fnSource}
