...
class VeskError extends Error {
  ...
  constructor(message, opts = {}) {
    ...
    this.tip = opts.tip || "";
    if (opts.code !== void 0) this.code = opts.code;
  }
  static notFound(name, candidates = [], context = {}) {
    const allCandidates = [.../* @__PURE__ */ new Set([...candidates, ...VESK_BUILTINS])];
    const suggestion = didYouMean(name, allCandidates);
    const isBuiltin = VESK_BUILTINS.includes(name);
    const msg = suggestion ? `"${name}" is not defined. Did you mean "${suggestion}"?` : `"${name}" is not defined.`;
    const nextSteps = [];
    ...
    return new VeskError(msg, {
      ...
      nextSteps,
      tip: isBuiltin ? `"${name}" is a Vesk built-in. Use it directly \u2014 no manual import needed.` : ""
    });
  }
