 ...
 * Vesk IR Generator
 *
 * Walks the parsed AST and produces IR nodes for server codegen.
 * Phase 2: expression-mode components only, non-reactive (server-only).
 */
import {
	IRRoot,
	ComponentIR,
...
function processJSXChildren(source, children) {
	...
	for (const child of children) {
		if (child.type === 'JSXText') {
			...
			// Slot: {props.children} or {children}
			if (
...
function processJSXElement(source, element) {
	...
	// Special handling for <Head> SEO metadata element
	if (tagName === 'Head') {
	...
	for (const attr of attributes) {
		...
		if (typeof attr.value === 'string') {
			...
			staticAttrs.push({ name: attr.name, value: '' }); // placeholder
			attrBindings.push(new DynamicBinding(attr.value, 'attribute', attr.name));
		}
	}
...
/**
 * Build IR for guard clauses + main return as nested conditionals.
 * This ensures short-circuiting: if a guard fires, later guards and the main return don't render.
 */
function buildGuardChain(source, guardClauses, mainReturn) {
	...
	// Build nested chain from back to front
		...
		for (const stmt of bodyStmts) {
			if (stmt.type === 'VeskBlock') {
				...
				if (stmt.tag === 'server') {
					...
					const rawName = elements.length > 1 ? elements[1]?.name : null;
					const init = getSource(source, stmt.declarations[0].init);
					if (name) preamble.push(new TrackDecl(name, init, rawName));
					const refName = getComponentRefName(stmt);
