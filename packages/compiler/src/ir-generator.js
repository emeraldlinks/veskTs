import { IRRoot, ComponentIR, StaticNode, TextNode, DynamicBinding, OpaqueDynamicRegion, MapRegion, WhileLoop, SwitchBlock, TryCatch, RuntimeStatement, ForLoop, TrackDecl, ComponentRef, ComponentCall, ServerBlock, ClientBlock, HeadBlock, Expression, SlotNode, } from './ir.js';
...
function extractKeyExpr(nodes) {
    for (const n of nodes) {
        if (n instanceof StaticNode && n.keyExpr)
            return n.keyExpr;
    }
    return null;
}
function isTrackDeclaration(decl) {
    return (decl.type === 'VariableDeclaration' &&
        ...
        decl.declarations[0].id.type === 'ArrayPattern' &&
        decl.declarations[0].id.lazy === true);
}
function getParamNames(params, source) {
    return params.map((p) => {
        ...
        if (p.type === 'ArrayPattern')
            return p.elements.map((el) => el?.name ?? '_');
        return ['_'];
    }).flat();
}
...
function processAttribute(source, attr) {
    ...
    if (attr.value.type === 'Literal')
        return { name, value: String(attr.value.value) };
    if (attr.value.type === 'JSXExpressionContainer') {
        const expr = attr.value.expression;
        if (expr.type === 'Literal')
            return { name, value: String(expr.value) };
...
function exprToIR(source, expr) {
    if (expr.type === 'JSXElement')
        return processJSXElement(source, expr);
...
function processJSXCallbackBody(source, body) {
    if (body.type === 'JSXElement')
        return processJSXElement(source, body);
    ...
    if (body.type === 'ParenthesizedExpression')
        return exprToIR(source, body.expression);
...
function hasJSXInSubtree(node) {
    if (!node)
        return false;
    if (node.type === 'JSXElement' || node.type === 'JSXExpressionContainer' || node.type === 'JSXFragment')
        return true;
    if (node.type === 'BlockStatement')
        return node.body.some(hasJSXInSubtree);
    if (node.type === 'IfStatement')
        return hasJSXInSubtree(node.consequent) || hasJSXInSubtree(node.alternate);
    if (node.type === 'ForStatement' || node.type === 'ForInStatement' || node.type === 'ForOfStatement')
        return hasJSXInSubtree(node.body);
    if (node.type === 'WhileStatement' || node.type === 'DoWhileStatement')
        return hasJSXInSubtree(node.body);
    if (node.type === 'SwitchStatement')
        return node.cases.some((c) => c.consequent?.some(hasJSXInSubtree));
    if (node.type === 'TryStatement')
        return hasJSXInSubtree(node.block) || hasJSXInSubtree(node.handler) || hasJSXInSubtree(node.finalizer);
    if (node.type === 'CatchClause')
        return hasJSXInSubtree(node.body);
    if (node.type === 'LabeledStatement')
        return hasJSXInSubtree(node.body);
    if (node.type === 'ReturnStatement')
        return hasJSXInSubtree(node.argument);
    return false;
}
...
function isStatementMode(bodyStmts) {
    if (bodyStmts.some((s) => s.type === 'JSXElement' || s.type === 'JSXExpressionContainer' || s.type === 'JSXFragment'))
        return true;
    for (const stmt of bodyStmts) {
        if (stmt.type === 'IfStatement' && !isGuardClause(stmt) && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'ForOfStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'ForStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'ForInStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'WhileStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'DoWhileStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'SwitchStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'TryStatement' && hasJSXInSubtree(stmt))
            return true;
        if (stmt.type === 'LabeledStatement' && hasJSXInSubtree(stmt))
            return true;
    }
    return false;
}
function processBlockBody(source, block) {
    if (block.type === 'BlockStatement')
        return processStatementModeBody(source, block.body);
    if (block.type === 'JSXElement')
        return processJSXElement(source, block);
    ...
    if (raw)
        return [new RuntimeStatement(raw, block, source)];
    return [];
}
...
function processForStatement(source, stmt) {
    if (stmt.type === 'ForOfStatement') {
        ...
        const itemVar = left.type === 'VariableDeclaration'
...
function processStatementModeBody(source, bodyStmts) {
    ...
    for (const stmt of bodyStmts) {
        if (stmt.type === 'JSXElement') {
            ...
            if (stmt.expression.type === 'JSXEmptyExpression')
                continue;
            ...
            if (name)
                nodes.push(new TrackDecl(name, init, rawName));
            const refName = getComponentRefName(stmt);
            if (refName)
                nodes.push(new ComponentRef(refName));
        }
            ...
            if (raw)
                nodes.push(new RuntimeStatement(raw, stmt, source));
        }
    }
    return nodes;
}
        ...
        if (isStatementMode(bodyStmts)) {
            ...
            for (const stmt of bodyStmts) {
                if (stmt.type === 'VeskBlock') {
                    ...
                    const init = getSource(source, stmt.declarations[0].init);
                    if (trackName)
                        preamble.push(new TrackDecl(trackName, init, rawName));
                    const refName = getComponentRefName(stmt);
                    if (refName)
                        preamble.push(new ComponentRef(refName));
                }
                    ...
                    const raw = getSource(source, stmt);
                    if (raw)
                        preamble.push(new RuntimeStatement(raw, stmt, source));
                }
            }
    ...
    function scanForAutoImport(nodes) {
        for (const node of nodes) {
            ...
            if (node instanceof DynamicBinding && node.expression && node.expression.raw) {
                for (const fn of autoImportable) {
                    if (node.expression.raw.includes(fn + '('))
                        usedFunctions.add(fn);
                }
            }
        ...
        for (const imp of imports) {
            ...
            if (match) {
                for (const n of match[1].split(','))
                    existing.add(n.trim().split(/\s+as\s+/).pop());
            }
        }
