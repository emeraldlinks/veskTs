...
export function matchApiUrl(tree, pathname) {
    ...
    function matchNodes(nodes, partIndex) {
        for (const node of nodes) {
            if (node.fullPath === '/') {
                if (partIndex >= parts.length && node.filePath)
                    return node;
...
export async function executeApiRoute(filePath, method, request, params = {}, locals = {}, devCache) {
    ...
    try {
        ...
        if (globalHookResult instanceof Response)
            return globalHookResult;
        let response;
        try {
        ...
        if (globalAfterResult instanceof Response)
            response = globalAfterResult;
