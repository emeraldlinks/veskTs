...
import { isStatic, prettifyHtml, resetVskState, loadRuntimeImports, evalTopLevelCode, callStaticProps, callLoadFunction, } from './server-utils.js';
...
export function render(source, componentName, props = {}, registry = new Map(), options = {}) {
    ...
    if (!renderFn)
        throw new Error(`Component "${componentName}" not found in source`);
...
export async function ssg(source, componentName, customProps, options = {}) {
    ...
    if (!componentName)
        throw new Error('No component found in source for SSG');
    ...
    const staticLists = ir.components.some((c) => {
        return c.body.some((node) => {
            if (node instanceof MapRegion)
                return isStaticIR(node.bodyTemplate);
            return false;
        });
    });
...
export async function renderFullPage(source, componentName, props = {}, registry = new Map(), options = {}) {
    ...
    try {
        ...
        if (cssLink)
            headLines.push(cssLink.trimEnd());
        if (headHtml)
            headLines.push('\t' + headHtml.split('\n').join('\n\t'));
        if (options.security) {
            const sec = options.security;
            if (sec.xFrameOptions !== false)
                headLines.push(`\t<meta http-equiv="X-Frame-Options" content="${sec.xFrameOptions || 'DENY'}" />`);
            if (sec.referrerPolicy !== false)
                headLines.push(`\t<meta name="referrer" content="${sec.referrerPolicy || 'strict-origin-when-cross-origin'}" />`);
            if (sec.contentSecurityPolicy !== false)
                headLines.push(`\t<meta http-equiv="Content-Security-Policy" content="${(sec.contentSecurityPolicy || "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; frame-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'").replace(/"/g, '&quot;')}" />`);
            if (sec.autoEscape !== false)
                headLines.push(`\t<!-- vesk: auto-escape enabled -->`);
        }
        return `<!DOCTYPE html>
<html>
<head>
...
export async function* renderPageStream(source, componentName, props = {}, registry = new Map(), options = {}) {
    ...
    if (options.security) {
        const sec = options.security;
        if (sec.xFrameOptions !== false)
            yield `\t<meta http-equiv="X-Frame-Options" content="${sec.xFrameOptions || 'DENY'}" />\n`;
        if (sec.referrerPolicy !== false)
            yield `\t<meta name="referrer" content="${sec.referrerPolicy || 'strict-origin-when-cross-origin'}" />\n`;
        if (sec.contentSecurityPolicy !== false)
            yield `\t<meta http-equiv="Content-Security-Policy" content="${(sec.contentSecurityPolicy || "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; frame-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'").replace(/"/g, '&quot;')}" />\n`;
        if (sec.autoEscape !== false)
            yield `\t<!-- vesk: auto-escape enabled -->\n`;
    }
    if (targetComp) {
