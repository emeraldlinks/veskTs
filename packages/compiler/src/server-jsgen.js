...
import { escapeHtml, indent, exprJS, childrenToHTML, extractTopLevelNames, extractRuntimeNames, buildParamInit, __vskHydrate, __vskImportedNames, setVskImportedNames, } from './server-utils.js';
export function irNodeToJS(node, importedNames, isAsync = false) {
    importedNames = importedNames || __vskImportedNames;
    if (node instanceof StaticNode)
        return staticNodeToJS(node);
    ...
    if (node instanceof DynamicBinding)
        return dynamicBindingToJS(node);
    if (node instanceof OpaqueDynamicRegion)
        return opaqueRegionToJS(node);
    if (node instanceof MapRegion)
        return mapRegionToJS(node);
    if (node instanceof WhileLoop)
        return whileLoopToJS(node);
    if (node instanceof SwitchBlock)
        return switchBlockToJS(node);
    if (node instanceof TryCatch)
        return tryCatchToJS(node);
    if (node instanceof ComponentRef)
        return '';
    if (node instanceof ComponentCall)
        return componentCallToJS(node, importedNames, isAsync);
    ...
    if (node instanceof SlotNode)
        return `__out.push(props.children || '');`;
    return '';
}
function staticNodeToJS(node) {
    ...
    const childNodes = node.children.filter((c) => !(c instanceof DynamicBinding && c.kind === 'attribute' && c.target !== 'ref'));
    if (hasDynamicAttrs) {
