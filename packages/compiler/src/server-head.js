...
function headElementKey(node, props, locals) {
    ...
    if (tag === 'meta') {
        if (attrMap.has('name'))
            return `meta[name=${attrMap.get('name')}]`;
        if (attrMap.has('property'))
            return `meta[property=${attrMap.get('property')}]`;
        if (attrMap.has('charset'))
            return 'meta[charset]';
        if (attrMap.has('http-equiv'))
            return `meta[http-equiv=${attrMap.get('http-equiv')}]`;
        return null;
    }
    if (tag === 'link') {
        if (attrMap.has('href'))
            return `link[href=${attrMap.get('href')}]`;
        if (attrMap.has('id'))
            return `link[id=${attrMap.get('id')}]`;
        return null;
    }
    if (tag === 'script') {
        if (attrMap.has('src'))
            return `script[src=${attrMap.get('src')}]`;
        return null;
    }
    if (tag === 'style')
        return null;
...
export function mergeHeadHtml(pageHead, layoutHead) {
    ...
    const extractKey = (tagStr) => {
        ...
        if (tagStr.startsWith('<meta')) {
            ...
            if (n)
                return `meta[name=${n[1]}]`;
            const p = tagStr.match(/\sproperty=["']([^"']+)["']/);
            if (p)
                return `meta[property=${p[1]}]`;
    ...
    for (const tag of layoutEntries) {
        ...
        if (key)
            merged.set(key, { html: tag, source: 'layout' });
    }
    const conflicts = [];
