/** @module completions — LSP completion providers for tags, attributes, expressions, imports, and CSS. */
...
/** Register `onCompletion` and `onCompletionResolve` handlers on the LSP connection. */
...
function buildAttributeCompletions(tagName: string, docUri: string, usedNames: Set<string>): CompletionItem[] {
  ...
  let props: string[] = [];
  let propTypes: Map<string, string> | null = null;
  if (isComponent) {
    const info = getComponentPropNames(tagName, docUri);
    props = info ? info.props : [];
    propTypes = getComponentPropsTypes(tagName, docUri);
  } else {
  ...
  for (const p of props) {
    ...
    seen.add(p);
    const typeStr = propTypes?.get(p);
    const typeDetail = typeStr ? `: ${typeStr}` : '';
    items.push({
      label: p,
      kind: CompletionItemKind.Property,
      detail: isComponent ? `Component prop${typeDetail}` : 'HTML attribute',
      documentation: typeStr ? { kind: MarkupKind.Markdown, value: `Type: \`${typeStr}\`` } : undefined,
      insertText: `${p}={$0}`,
      insertTextFormat: InsertTextFormat.Snippet,
    });
  }
