...
import type { ApiRouteNode, ApiFunctionOptions } from './types.js';
...
export function generateApiFunction(
  ...
  if (middlewareCode) {
    handleBody = [
      ...
      '  try {',
      `    const handler = { ${handlerMethods} }[method as keyof typeof ${handlerMethods}];`,
      '    if (!handler) {',
      ...
      "      if (typeof (response as any).build === 'function') (response as any).build();",
      '      return response;',
      '    }',
      ...
      '      return new Response(null, { status: err.status || 302, headers: { Location: err.url! } });',
      '    }',
    ...
    handleBody = [
      ...
      '  globalThis.__vesk_request = ctx;',
      '  try {',
      `    const handler = ((${handlerMethods} as unknown) as Record<string, Function>)[method];`,
      '    if (!handler) {',
      "      return new Response(JSON.stringify({ error: 'Method not allowed' }), {",
      '        status: 405,',
      "        headers: { 'Content-Type': 'application/json' },",
