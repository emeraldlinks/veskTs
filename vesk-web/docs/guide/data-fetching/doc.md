...
/** Progressive streaming — chunk-by-chunk into a Tracked cell. */
useFetch.stream(
  urlOrFn: string | (() => string),
  options?: Omit<UseFetchOptions<string>, 'body'> & {
    into?: Tracked<string>;                        // cumulative text after each chunk
    onChunk?: (chunk: string, total: string) => void;
  }
): Resource<string>; // + .into === options.into
// urlOrFn may be a provider () => string re-evaluated on every fetch/refresh().
// Supply `key` when the URL is dynamic so handles co-locate (e.g. key: 'doc').
```

### Streaming — useFetch.stream

Streams `res.body` via `TextDecoder` into `into` (`streamText` in `packages/runtime/src/resource.ts`): `set(into, total)` after each chunk so `<Md>` re-renders progressively. Falls back to `res.text()` when `getReader()` unavailable. `HttpError` on `!res.ok`.

* **Provider re-evaluated:** `() => '/api/docs/' + docPath` is called on every fetch, including `res.refresh()`, so `const &[docPath] = track('welcome')` → `docPath = 'guide'` propagates without recreating the resource.
* **API vs file paths:** API param has no extension (`'welcome'`), `<Md>` public file **needs** `.md` (`'/game.md'`, `'/welcome.md'`).
...
component Docs {
  ...
  const &[doc] = track('')
  const res = useFetch.stream(() => '/api/docs/' + docPath, {
    into: doc,
    key: 'doc',
    onChunk: (chunk, total) => console.log(chunk.length, total.length)
  })

  <div class="flex gap-2">
    <select class="border rounded px-2 py-1" onchange={(e) => { docPath = (e.target as HTMLSelectElement).value; res.refresh() }}>
      <option value="welcome">welcome</option>
      <option value="guide/getting-started">getting-started</option>
    </select>
    <span class="text-sm text-gray-500">{res.loading ? 'Loading…' : 'Ready'}</span>
  </div>
  <Md content={doc} css />           // streams chunk-by-chunk
  <Md content="/welcome.md" css />  // public file — needs .md + leading /
}
```
