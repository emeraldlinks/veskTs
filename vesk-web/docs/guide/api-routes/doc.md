...
VeskResponse.stream(readable, init?)       // chunked ReadableStream (Transfer-Encoding: chunked)
                                           // readable as unknown as BodyInit; dev/prod via deliverResponse
...
Streaming example — docs API in `vesk-web/app/api/docs/[...path]/route.ts` (verified in `packages/runtime/src/request.ts` and `packages/adapter/src/dev-server.ts`):

```ts
// app/api/docs/[...path]/route.ts — streams markdown in 400-byte chunks
const stream = new ReadableStream<Uint8Array>({
  start(controller) {
    const timer = setInterval(() => {
      if (offset >= content.length) { clearInterval(timer); controller.close(); return; }
      controller.enqueue(encoder.encode(content.slice(offset, offset + 400)));
      offset += 400;
    }, 8);
  }
});
return VeskResponse.stream(stream, { headers: { 'Content-Type': 'text/markdown; charset=utf-8' } });
// or: return new Response(stream, { headers: { 'Content-Type': 'text/markdown' } });
```
