 ...
 * cell or streamed resource. Defaults differ from raw renderMarkdown():
 * highlight/ids/autolink default ON here. Source confirmed in
 * packages/types/src/index.ts (MdProps) and packages/runtime/src/md.ts.
 */
function Md(props: {
	content?: string | Tracked<string> | Resource<string>;
	    // string — literal markdown
	    // Tracked<string> — reactive cell (const &[x] = track(''))
	    // Resource<string> — streamed via useFetch.stream (unwraps .into)
	    // string with .md — absolute public path "/welcome.md", "/game.md", "/docs/guide.md"
	    //   (server: __vsk_md_read_file hook, client: fetch + mdPathCache, literal fallback)
...
Reactive content — live re-render per keystroke (auto-tracked `&` — no `get`):

```vsk
import { Md } from '@vesk/runtime';

component Live() {
  const &[src] = track("# Hello")
  <Md content={src} theme="dark" css />
}
```

Streamed content — re-renders per chunk:
...
component Streamed() {
  const &[doc] = track('')
  const res = useFetch.stream(() => '/api/docs/welcome', { into: doc })
  <Md content={doc} css />
}
```

Public file — needs `.md` + leading `/` (under `public/`; `//`, `?`, `#`, `\` rejected):

```vsk
<Md content="/welcome.md" css />
<Md content="/game.md" css />
<Md content="/docs/guide.md" css />
// "/welcome" or "game.md" → renders literally, not a file
```
