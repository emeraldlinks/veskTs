...
**Completed this session (agentic keys):** Agentic refocused from single `VESK_AGENTIC_API_KEY` env model to per-provider `.env.local` `VK_{PROVIDER}_KEY` variables read at dev time and surfaced in the Settings→Agentic UI (set/unset per provider). New `config.ts` primitives (`providerDotenvVar`, `dotenvPath`, `readDotenvValue`, `writeDotenvValue`, `getApiKey`, `saveApiKey`, `hasKey`, `listApiKeys`) read/write/update-in-place `.env.local`; `saveApiKey` writes `VK_*_KEY`, 2-arg form targets configured provider. `dev-api.ts` `/keys`+`/config` now reflect set/unset per provider; `cli/dev-server.ts` + `adapter/dev-server.ts` gate changed from `VESK_AGENTIC_API_KEY` to "any VK_*_KEY present OR agentic plugin active", and `runAgent`/`runAgenticAgentMain` resolve apiKey via `getApiKey(projectDir, providerName)` instead of env vars. All `VESK_AGENTIC_API_KEY`/`OPENCODE_API_KEY` references removed. Verified live on vesk-web: key present → gate open + 63 opencode models; key absent → 404 gate; POST `/config`+`/keys` writes/replaces `VK_*_KEY` in `.env.local` in place. Added `config.test.ts` (19 tests). Repo typecheck clean.
